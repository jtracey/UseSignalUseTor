This requires quite a few pieces to get working properly.
 - The Android sdk. This can be obtained either with Android Studio, or the standalone CLI tools.
   - https://developer.android.com/studio (Command line tools are near the bottom.)
 - The following Android sdk components, which can be installed either via Android Studio's SDK Manager or the sdkmanager CLI tool:
   ...
   - See https://developer.android.com/tools/sdkmanager for installing all the above with the CLI (adb is in platform-tools)
 - Magisk APK. Used to get an accessible su binary on the emulator, and to provide a framework for allowing some additional mucking around with the installed apps. Tested: v25.2 https://github.com/topjohnwu/Magisk/releases
 - MagiskOnEmulator. A small set of tools used to get Magisk working on an emulator. Tested: Custom revision (`./MagiskOnEmulator/`)
 - Termux APK. Used to get a package manager and more fully-featured shell environment on Android. Tested: termux-app_v0.118.0+github-debug_x86.apk https://github.com/termux/termux-app/releases
 - SSH. Used to access Termux from the host shell.
 - sshd, tsu, tcpdump, mitmproxy packages on Termux. Used to allow ssh, sudo, packet captures, and MITM'ing traffic in android. (These are installed as part of the instructions.)
 - frida: Used to modify executables. Tested: 16.0.19 https://frida.re/docs/installation/
 - magisk-frida: Used to allow Frida to modify running apps via Magisk. Tested: 16.0.19-1 https://github.com/ViRb3/magisk-frida/releases
 - The APK for the app you'll be MITM'ing. Because root is not (easily) available on emulators with the Play Store, you'll likely need to find some source to download it from. Signal has APKs available here: https://signal.org/android/apk/
...
MITM'ing android app traffic:
 ...
 - Cold start the emulator (*must be a cold start!* If using Android Studio, do not use the power button on the emulator, use the extended controls; if using the CLI, add `-no-snapshot-load` to your emulator command)
 ...
 - inside termux, install the root-repo, upgrade, then binutils, python, python-pip, python-cryptography [FIXME: do we?], rust, openssh, screen, tsu, frida, and tcpdump,  (`pkg install root-repo && pkg upgrade -y && pkg install binutils python python-pip rust openssh screen tsu frida tcpdump -y`)
...
To view the MITM'd data with Wireshark:
 - Ensure that reassembly is fully enabled:
   Edit -> Preferences -> Protocols -> TCP (n.b.: you can type the name to jump to the protocol), ensure "Allow subdissector to reassemble TCP streams" and "Reassemble out-of-order segments" are enabled.
 - Enable TLS decryption
   Edit -> Preferences -> Protocols -> TLS, ensure all "Reassemble..." options are enabled, then select the pulled key file in "(Pre)-Master-Secret log filename".
 - Open the pcap file.
 - Filter to "(http or http or websocket) and ip.addr != 127.0.0.1" to see the most relevant application-layer data. To view accurate timing and order information about packets instead, you'll need to disable the TCP and TLS reassemble options mentioned above, which will render much of the application data inaccessible.
 - To parse the protobufs in Signal messages:
   - Edit -> Preferences -> Protocols -> ProtoBuf
     Edit the search paths to include the `protobufs` directory of this project, and ensure "Dissect Protobuf fields as Wireshark fields" and "Show details of message, fields and enums" are enabled.
